...
# make a bfile
${xwasloc}/plink_xwas --bfile ${fname} --make-bed --keep ${fname}_rand200controls.keep --out ${fname}_rand200controls --noweb  --silent
...
do
	...
	then
		...
		awk ' ($4 > '$buffstart') && ($4 < '$buffstop') {print $2}' ${fname}_rand200controls.bim > ${fname}_${gene}.snps
		...
		then
			...
			# extract the snps
			${xwasloc}/plink_xwas --bfile ${fname}_rand200controls --extract ${fname}_${gene}.snps --out temp --make-bed --silent --noweb
			...
			# get the LD matrix
			${xwasloc}/plink_xwas --bfile temp --matrix --r --extract temp.snp --noweb --out ${fname}_ld --silent
			# replacing nan values with 0
			sed -i 's/nan/0/g' ${fname}_ld.ld 
...
rm ${fname}_rand200controls.bed ${fname}_rand200controls.bim ${fname}_rand200controls.fam ${fname}_rand200controls.log temppv.txt
